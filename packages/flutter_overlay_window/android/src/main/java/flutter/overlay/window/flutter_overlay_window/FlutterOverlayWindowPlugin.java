...
public class FlutterOverlayWindowPlugin implements
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        ...
        if (call.method.equals("checkPermission")) {
            ...
            // Plain startService() throws IllegalStateException on API 26+
            // when the app has no foreground component at call time — which
            // is exactly when this now runs, since the app shows the bubble
            // right as it leaves the foreground (see
            // OverlayService.showIfEnabledAndPermitted() on the Dart side).
            // startForegroundService() is explicitly allowed from the
            // background; it just requires the service to call
            // startForeground() promptly, which OverlayService.onCreate()
            // already does.
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (OverlayService.isRunning) {
                ...
                // Without this branch, calling closeOverlay() while the
                // overlay isn't running never calls back to Dart at all, so
                // `await FlutterOverlayWindow.closeOverlay()` hangs forever.
                // That case is routine once the app hides the bubble on
                // every foreground resume — most of those calls land when
                // it's already not showing.
